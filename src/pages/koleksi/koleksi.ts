...
@Component({
  selector: 'page-koleksi',
  templateUrl: 'koleksi.html'
})
export class KoleksiPage {
  data: any;
  summary: any;
  loading: any;
  page = 1;
  ...
  getData() {
    this.showLoader();
    this.authService.getListPage('koleksi', this.page).then((result) => {
      this.data = result;
      this.page = this.page + 1;


      this.authService.getSummary('koleksi').then((result) => {
        this.summary = result;
